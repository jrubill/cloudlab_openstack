...
openstack server create --flavor m1.medium --security-group $security_id --image OL7 --nic port-id=$port_id sub1

port_id=`openstack port list -f value | grep node2 | cut -d' ' -f 1`
openstack server create --flavor m1.medium --security-group $security_id --image OL7 --nic port-id=$port_id sub2

port_id=`openstack port list -f value | grep node1 | cut -d' ' -f 1`
openstack server create --flavor m1.medium --security-group $security_id --image OL7 --nic port-id=$port_id sub3
